...
import re
import sys
sys.path.append("/path/to/resources")
import subprocess
...
# TODO: 
# - [high] Automatically add file suffix when saving.  保存时自动添加名称以及后缀   #####@  TODO
# - [high] Add polygon movement with arrow keys  移动多边形
# - [high] Deselect shape when clicking and already selected(?)  绘制的图形过程中有取消上一点或者整个绘制图形选项？ 
# - [high] Sanitize shortcuts between beginner/advanced mode.   高级和初级模式的切换
# - [medium] Zoom should keep the image centered.   以图像中心点缩放图片
# - [medium] Add undo button for vertex addition.   添加顶点绘制取消选项
# - [low,maybe] Open images with drag & drop.     拖拽加添图片
# - [low,maybe] Preview images on file dialogs. 软件中预览图片 
# - [low,maybe] Sortable label list.  分类标签列表
# - Zoom is too "steppy". 缩放跨步太大
    ...
    def __init__(self, filename=None):
        ...
        quit = action('&Quit', self.close,
               # 'Ctrl+Q', 'quit', u'Quit application')
                'Q', 'quit', u'Quit application')   #change shortcut key  HT
        open = action('&Open', self.openFile,
                'Ctrl+O', 'open', u'Open image or label file')
        save = action('&Save', self.saveFile,
                # 'Ctrl+S', 'save', u'Save labels to file', enabled=False)
                'S', 'save', u'Save labels to file', enabled=False)  # #change shortcut key  HT
        ...
        next = action('Next', self.openNextImg,#self.openFile,
                'T', 'next', u'Start next image')
        ...
        create = action('Create\nPolygo&n', self.createShape,
                'N', 'new', u'Draw a new polygon', enabled=False)
        ...
        # self.zoomMode = self.FIT_WINDOW  #HT DO
        self.scalers = {
        ...
        self.actions = struct(save=save, saveAs=saveAs, open=open, next=next, close=close,
                ...
                fileMenuActions=(open,next,save,saveAs,close,quit),
                beginner=(), advanced=(),
        ...
        addActions(self.menus.file,
                (open, next, self.menus.recentFiles, save, saveAs, close, None, quit))
        ...
        self.actions.beginner = (
            open, save, None, next, create, copy, delete, None,
        ...
        size = settings.get('window/size', QSize(1920, 1080))
...
#                self.lineColor.getRgb(), self.fillColor.getRgb())
    ...
    def scaleFitWindow(self):
        ...
        e = 2.0 # So that no scrollbars are generated.     /// e = 2.0
    ...
    def openNextImg(self, _value=False):
        all_img_count = find_image_count(imgFilePath)
        if all_img_count == 0:
            return
        global current_count
        countImgName = '{:0>6}.jpg'.format(current_count)
        print countImgName.replace('.jpg', '')
        imgPath = os.path.join(imgFilePath, countImgName)
        print'现在处理第{}张图片,剩余{}张'.format(current_count + 1, (all_img_count - current_count - 1))
        if imgPath:
            self.loadFile(imgPath)
        current_count += 1

    def saveFile(self, _value=False):
        assert not self.image.isNull(), "cannot save empty image"
        if self.hasLabels():
            self.saveFileDirect(imgFilePath)
...
#            self._saveFile(self.saveFileDialog())
    ...
    def saveFileDirect(self,filePath):
        # 文件保存路径为当前处理图片所在路径
        # 写入的文件名与当前处理的图片名相同
        countImgName = '{:0>6}.jpg'.format(current_count)
        imgname = countImgName.split('.',1)[0]
        lifname = '{:0>6}.lif'.format(imgname)
        savelifname = os.path.join(filePath,lifname)
        self.saveLabels(savelifname)


    def saveFileDialog(self):  #  调用QT的窗口函数命名标注文件
        caption = '%s - Choose File' % __appname__
        filters = 'Label files (*%s)' % LabelFile.suffix
        dlg = QFileDialog(self, caption, self.currentPath(), filters)
        dlg.setDefaultSuffix(LabelFile.suffix[1:]) #保存默认后缀'.lif'
        dlg.setAcceptMode(QFileDialog.AcceptSave) #状态是保存文件
        dlg.setConfirmOverwrite(True)
        dlg.setOption(QFileDialog.DontUseNativeDialog, False) #自定义目录过滤
        if dlg.exec_():
...
current_count = 0
imgFilePath = '/home/herobot/clothData/2017_09_04/clothRed/source/jpg'
